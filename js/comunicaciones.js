/* ============================================================
   SUPERVISION-FLANDES · SOLICITUD A COMUNICACIONES (antes PRENSA)
   ...
   Supervisión (dictadas por Oss el 23/09/2026):
     ...
     · Quien pide es el supervisor (o el revisor), no un contrato: la
       ...
       primero.
   La regla la hace cumplir el SERVIDOR (SUPERVISION.comunicacionesSolicitar);
...
(function () {
  ...
  var K = window.KIT;
  var C = {};
  var P = null;              /* lo último que respondió comunicacionesEstado */
  var FILTRO = '';
  ...
  function vista(sub) {
    ...
    app().appendChild(c);
    var p = K.pedir('comunicacionesEstado', {}, { ms: 60000 }).then(function (r) { P = r; return r; });
  ...
  function mias() {
    ...
    if (!sol.length) {
      s.appendChild(K.nodo('<p class="seg-nada">Todavía no le has pedido nada a Comunicaciones desde esta app.</p>'));
      return s;
    }
